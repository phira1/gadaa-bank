...
const LookingFor = () => {
  ...
  const services = [
    ...
    { 
      ...
      path: 'https://play.google.com/store/apps/details?id=com.gadaaDigital',
      external: true
    },
    { 
      icon: FaNewspaper, 
  ...
  // Helper to render link or external anchor
  const renderLink = (service, index) => {
    const commonClasses = `block bg-gray-50 rounded-lg p-3 md:p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-transparent hover:border-red-500 cursor-pointer group ${
      animatedItems[index] ? 'animate-fadeInUp opacity-100' : 'opacity-0'
    }`;
    const style = animatedItems[index] ? { animationDelay: `${index * 50}ms` } : {};
      ...
      <div className="container mx-auto px-3 md:px-4">
        ...
        <div className="max-w-6xl mx-auto mt-6 md:mt-12 px-1">
          <div className="grid grid-cols-2 gap-3 md:gap-6 md:grid-cols-2">
            {services.map((service, index) => renderLink(service, index))}
          </div>
        </div>
      </div>
